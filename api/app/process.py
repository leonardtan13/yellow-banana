...
def process_request(request_body):
    ...
    df_data['MonthlyCharges'] = pd.to_numeric(df_data['MonthlyCharges'], downcast='float')
    df_data['TotalCharges'] = pd.to_numeric(df_data['TotalCharges'], downcast='float')

    
    df_data['gender'] = df_data['gender'].transform(lambda x: 1 if x == 'Male' else 0)
    ...
    for column in df_data.columns:
        if df_data[column].dtype == object and (column != 'gender' or column not in binary_cols):
